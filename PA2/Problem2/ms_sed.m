function [labels,clusters] = ms_sed(XX,hc1,hc2,hp1,hp2,h)
...
for i = 1:n
    ...
    while true
        ...
        for ii = unique(pt_in_h)
            dis_c1 = (XX(1,ii)-old_miu(1))'*(XX(1,ii)-old_miu(1));
            dis_c2 = (XX(2,ii)-old_miu(2))'*(XX(2,ii)-old_miu(2));
            dis_p1 = (XX(3,ii)-old_miu(3))'*(XX(3,ii)-old_miu(3));
            dis_p2 = (XX(4,ii)-old_miu(4))'*(XX(4,ii)-old_miu(4));
            kernel = (1/4*(pi^2)*(hc1^2)*(hc2^2)*(hp1^2)*(hp2^2))*...
                        exp(-0.5*(1/hc1^2)*dis_c1...
                        -0.5*(1/hc2^2)*dis_c2...
                        -0.5*(1/hp1^2)*dis_p1...
                        -0.5*(1/hp2^2)*dis_p2);
